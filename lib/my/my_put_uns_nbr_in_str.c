...
static int is_nb_negative(unsigned long long int nb)
{
    if (nb < 0) {
        return -1;
    } else if (nb == 0)
...
static int get_nb_lenght(unsigned long long int nb, int signe)
{
    int lenght = 0;
...
char *my_put_uns_nbr_in_str(unsigned long long int nb)
{
    ...
    unsigned long long int nb_origin = nb * signe;
