...
static int nb_negative(unsigned long long int nb)
{
    if (nb < 0) {
        my_putchar('-');
        return -1;
...
static int nb_lenght(unsigned long long int nb, int signe, int base_lenght)
{
    int lenght = 0;
...
char *my_putunsnbr_base_in_str(unsigned long long int nbr, char const *base)
{
    ...
    unsigned long long int nb_origin = nbr * signe;
